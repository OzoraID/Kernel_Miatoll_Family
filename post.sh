...
NAME_KERNEL="Kurumi-Alpha"
BASE="Kinesis"
KERNEL_DIR="$PWD"
KERNEL_IMG="$KERNEL_DIR/out/arch/arm64/boot/Image"
KERNEL_DTBO="$KERNEL_DIR/out/arch/arm64/boot/dtbo.img"
KERNEL_DTB="$KERNEL_DIR/out/arch/arm64/boot/dtb.img"
KBUILD_COMPILER_STRING=$(/workspace/ehhe/clang/bin/clang --version | head -n 1 | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g' -e 's/[[:space:]]*$//')
AK3_DIR="$KERNEL_DIR/AK3"
PHONE="Miatoll Family"
DEVICE="Miatoll"
...
push() {
    ...
    curl -F document=@"$AK3_DIR/$ZIP_NAME" "https://api.telegram.org/bot$TOKEN/sendDocument" \
        ...
        -F "parse_mode=html" \
        -F caption="It's time to brick | <b>Miatoll</b>"
}
